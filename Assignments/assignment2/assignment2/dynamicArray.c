...
int _getIndex(struct DynArr * v, TYPE val)
{
    assert(v != 0 && v->size > 0);
    int i;
    for(i = 0; i < v->size; i++)
...
{
    assert(v != 0 && v->size > 0 && i >= 0 && j >= 0 && i < v->size && j < v->size);
    TYPE temp = v->data[i];
    v->data[i] = v->data[j];
    v->data[j] = temp;
}
...
{
    assert(v != 0 && v->size > 0 && containsDynArr(v, val));
    removeAtDynArr(v, _getIndex(v, val));
}
