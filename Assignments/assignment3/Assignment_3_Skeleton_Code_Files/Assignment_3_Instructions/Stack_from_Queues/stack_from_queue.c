...
{
    assert(queue != 0);
    struct Link *sentinel = (struct Link *) malloc(sizeof(struct Link)); assert(sentinel != 0);
    sentinel->next = 0;
    queue->head = sentinel;
    queue->tail = sentinel;
}
...
{
    struct Queue *queue = (struct Queue *) malloc(sizeof(struct Queue)); assert(queue != 0);
    listQueueInit(queue);
    return queue;
}
...
TYPE listQueueRemoveFront(struct Queue* queue)
{
    ...
    TYPE valueToReturn = queue->head->next->value;
...
{
    struct Stack *newStack = (struct Stack *) malloc(sizeof(struct Stack)); assert(newStack != 0);
