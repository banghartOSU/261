...
{
    assert(val != 0);
    if(cur == 0) //Base case - we've found a place for the new node.
    {
        struct Node *newNode = (struct Node *) malloc(sizeof(struct Node)); assert(newNode != 0);
        newNode->val = val;
        newNode->left = 0;
        newNode->right = 0;
        return newNode;
    }
    else if(compare(val, cur->val) < 0) //If the new value is less than current, slide left.
    {
        cur->left = _addNode(cur->left, val);
    }
    else  //If the new value is greater than the current, slide right.
    {
        cur->right = _addNode(cur->right, val);
    }
    return cur; //Always return a reference to the current node.
}
...
int containsBSTree(struct BSTree *tree, TYPE val)
{
    ...
    while(current != 0)
    {
        if(compare(val, current->val) == 0) {return 1;} //Found the same value
        else if(compare(val, current->val) < 0) {current = current->left;} //Move left;
...
struct Node *_removeLeftMost(struct Node *cur)
{
    if(cur->left == 0)
    {
        struct Node *toReturn = cur->right;
        free(cur);
        return toReturn;
    }
    else
    {
        ...
        return cur; //Is this necessary at all?
    }
}
/*
...
{
    if(compare(val, cur->val) == 0) //Found value to remove
    {
        if(cur->right == 0) {return cur->left;} //Special case to right subtree
        else
        {
