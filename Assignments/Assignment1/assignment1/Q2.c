...
//    coming from a C++ background. This is beacuse there is no pass by reference in C -- here we are
//    passing a copy of the addresses of each variable - not the addresses themselves. The correct way
//    to make this swap work would be to define foo() as foo(int** a, int** b, int c).
//    This would provide us with a pointer to the pointers from which we could access the
//    memory address of each pointer. We would pass the memory address of pointers to foo() given that
//    definition.
// */
//
